...
def load_dataset_2class(e_file_list, tau_file_list,
...
def load_dataset_2class_vars(e_file_list, tau_file_list,
                        n_training, n_validation,
                        img_shape,var_shape,
                        get_weights=False,
                        weight_type='total',
    ...
    for n_to_load in [n_training, n_validation]:
        # load n_to_load for each flavor
        img_array = np.empty((2*n_to_load,) + img_shape, dtype=np.float32)
        vars_array = np.empty((2*n_to_load,) + var_shape, dtype=np.float32)
        
        if get_weights:
            weights = np.empty(2*n_to_load, dtype=np.float32)

        out_e = load_n_images_vars(e_file_list,
                              n_to_load,
                              img_array,
			      vars_array,
                              get_weights,
        ...
        out_tau = load_n_images_vars(tau_file_list,
                                n_to_load,
                                img_array[n_to_load:],
                                vars_array[n_to_load:],
                                get_weights,
                                weight_type,
                                spectral_index,
                                verbose)
        ...
        if get_weights:
            # copy weights into output array
            weights[:n_to_load] = out_e[-2]
            weights[n_to_load:] = out_tau[-2]

            output_arrays.append((img_array, vars_array, weights))

        else:
            output_arrays.append((img_array, vars_array))

    return tuple(output_arrays), (e_file_list, tau_file_list)

def load_dataset_3class(e_file_list, mu_file_list, tau_file_list,
                        n_training, n_validation,
                        img_shape,
                        get_weights=False,
                        weight_type='total',
    ...
    for n_to_load in [n_training, n_validation]:
        # load n_to_load for each flavor
        img_array = np.empty((3*n_to_load,) + img_shape,
                             dtype=np.float32)

        if get_weights:
            weights = np.empty(3*n_to_load, dtype=np.float32)
        ...
        out_mu = load_n_images(mu_file_list,
                               n_to_load,
                               img_array[n_to_load:],
                               get_weights,
                               weight_type,
                               spectral_index,
                               verbose)
        mu_file_list = out_mu[-1]

        out_tau = load_n_images(tau_file_list,
                                n_to_load,
                                img_array[2*n_to_load:],
                                get_weights,
                                weight_type,
                                spectral_index,
                                verbose)
        ...
        if get_weights:
            ...
            weights[:n_to_load] = out_e[1]
            #weights[:n_to_load] = out_e[1]
            weights[n_to_load,2*n_to_load] = out_mu[1]
            weights[2*n_to_load:] = out_tau[1]
    ...
    return tuple(output_arrays), (e_file_list, mu_file_list, tau_file_list)

def load_n_images_vars(file_list, n, img_array=None, vars_array =None,
    ...
    for i, file_name in enumerate(file_list):
        ...
        if img_array is None:
            ...
            vars_array = np.empty((n,) + batch_2[0].shape, dtype=np.float32)
        ...
        if len(batch) <= len(view):
            #len(batch)=len(batch_2)
            view[:len(batch)] = batch
            view = view[len(batch):]
            view_2[:len(batch)] = batch_2
            view_2 = view_2[len(batch):]
    ...
    if get_weights:
        return img_array, vars_array, weights, file_list[i+1:]

    else:
        return img_array, vars_array, file_list[i+1:]
...
def train_with_callback_2class_vars(model, initial_rate, 
                               momentum, patience,
                               n_epochs, callback,
                               dataset, batch_size,
                               ratio = [1,1],
    ...
    if len(training_data) == 3:
        training_data, training_vars, training_weights = training_data
        validation_data, validation_vars, validation_weights = validation_data
    else:
        training_data, training_vars = training_data
        validation_data, validation_vars = validation_data
        training_weights = None

    print("AAA",training_data.shape,training_vars.shape,validation_data.shape,validation_vars.shape)
   # print(np.stack((training_data,training_vars.reshape(training_vars.shape[0],training_vars.shape[-1]))).shape)
    ...
    if input_adapter is not None:
        training_data = input_adapter(training_data,training_vars)
        validation_data = input_adapter(validation_data,validation_vars)
        validation_tuple = (validation_data,) + validation_tuple[2:]
        print("aaa",[i.shape for i in validation_data])
        print("bbb",[len(i) for i in validation_tuple])
    return model.fit(
...
def train_with_callback_2class(model, initial_rate, 
                               momentum, patience,
                               n_epochs, callback,
                               dataset, batch_size,
                               ratio = [1,1],
...
def train_with_callback_3class(model, initial_rate,
                               momentum, patience,
                               n_epochs, callback,
                               dataset, batch_size,
                               ratio = [1,1,1],
    ...
    n_train, n_valid = [int(len(data)/3)
                        for data in (training_data, validation_data)]

    training_labels, validation_labels = [
        np.array([[1,0,0]]*n + [[0,1,0]]*n + [[0,0,1]]*n) for n in [n_train, n_valid]]
    ...
    return model.fit(
        ...
        epochs=n_epochs,
        callbacks=callbacks,
        class_weight={0:ratio[0],1:ratio[1],2:ratio[2]},
        validation_data=validation_tuple,
        sample_weight=training_weights,
        shuffle=True).history
    # print LR callback from
...
def get_all_scores_vars(model, img_array, var_array, input_adapter=None):
    ...
    if input_adapter is not None:
        img_array = input_adapter(img_array,var_array)
...
def test_model_on_file_vars(model,
    ...
    for nvar in range(nvars):
        dvars[:,0,nvar] = np.clip(data['vars'][:,0,nvar],clim[nvar][0],clim[nvar][1])
        dvars[:,0,nvar] = np.clip(data['vars'][:,0,nvar],clim[nvar][0],clim[nvar][1])
        mean_vars[nvar] = np.mean(dvars[:,0,nvar])
        std_vars[nvar] = np.std(dvars[:,0,nvar])
        dvars[:,0,nvar] = (dvars[:,0,nvar] - mean_vars[nvar])/std_vars[nvar]
        dvars[:,0,nvar] = (dvars[:,0,nvar] - mean_vars[nvar])/std_vars[nvar]
    #print(mean_vars,std_vars)
    #print(dataset[0][1][0])
    norm_img_array = normalized(data['image'], True,
                                norm_mode, mean, std)
    output_table[:, 0] = get_all_scores_vars(model, norm_img_array, dvars, input_adapter)
