...
from cnn_common.util_3class import *
import time
import sys
...
def build_and_train_model(n_training,
    ...
    print('\nLoading raw data...\n')
    start = time.time()
    dataset, unconsumed_files = load_dataset(e_file_list, tau_file_list, mu_file_list,
...
def characterize_model(model, e_file_list, tau_file_list, mu_file_list,
		       input_adapter=None,
    ...
    for file_list in [e_file_list, tau_file_list,mu_file_list]:
        tables.append(
    ...
    validation_files_per_flavor = [len(l)
                                   for l in [e_file_list, tau_file_list,mu_file_list]]
