...
def load_dataset(e_file_list, tau_file_list,mu_file_list,
    ...
    for n_to_load in [n_training, n_validation]:
        ...
        img_array = np.empty((4*n_to_load,) + img_shape,
                             dtype=np.float32)

        if get_weights:
            weights = np.empty(4*n_to_load, dtype=np.float32)
...
def train_with_callback(model, initial_rate, momentum, patience,
    ...
    n_train, n_valid = [int(len(data)/4)
                        for data in (training_data, validation_data)]

    training_labels, validation_labels = [
        np.array([[1,0,0]]*n + [[0,1,0]]*n + [[0,0,1]]*2*n) for n in [n_train, n_valid]]
...
def annual_weights_from_data(data,nu_type="e", spectral_index=DEFAULT_INDEX):
    ...
    return get_annual_weights(table[:, 0], table[:, 1], table[:,2], i3_per_npz = i3_per_npz,
...
def per_year_past_cut(table, score_cut, n_files, nu_type ="e"):
    ...
    return get_annual_weights(cut_table[:, 1], cut_table[:, 2], cut_table[:,3], i3_per_npz=i3_per_npz, n_npz_files = n_files).sum()
