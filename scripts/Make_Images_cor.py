...
info_dtype = np.dtype(
	...
	("primary_child_pdg", np.float32,(N_PRIM_CHILDREN)),
        ("muon_veto", veto_dtype),   #logan veto variables                                                                                                 
	("hese", hese_dtype),                                                               
	("weight", weight_dtype)
...
#    print("AAAAA",we)
    ...
    event[["id","image","qtot","qst","qst_all","primary","prim_daughter","primary_child_energy","primary_child_pdg","logan_veto","hese_old","hese","weight"]]=(id[0],im,qtot,st_info,st_info_all,primary[0],prim_daughter[0],energies,pdgs,veto[0],hese[0],hese[1],weight[0])
    print("AAAA", event['id'])
    data.append(event)
