...
#    print("AAAAA",we)
    ...
    for img_ch, (q, stnum, dist) in enumerate(st_info):
		...
		for wf in wf_map.get(omkey, []):
			...
			wf2.append([wf.time,img_ch,omkey.om - 1])
			wfms.append({
				'wfm': wf.waveform,
                                'time': wf.time,  
                                'width': wf.bin_width,
    ...
    wf2 = np.array(wf2)
    wf2 = wf2[wf2[:,0].argsort()]
   
    #print(wf_times.shape)
    sz = 180 - wf2.shape[0]
    z = np.zeros((sz,3))
    #print(wf2.shape,z.shape)
    wf2 = np.concatenate((wf2,z),axis =0)
    #print(wf_times)
 
    diff_times = np.diff(wf_times[:3])
    #print("AAA",diff_times)
    max_diff_pos = np.argmax(diff_times)
  #  print("CCC",diff_times[max_diff_pos],diff_times)
    if diff_times[max_diff_pos] > 700:
	...
	if wf_shift > 0:
	    print(start_ind,end_ind,wf_shift,len(wfm_vals))
            print(np.true_divide(wfm['wfm'],10**(-11)))
	    print(np.true_divide(wfm_vals,10**(-11)))
        #im[start_ind:end_ind, wfm['dom_idx'], wfm['img_ch']] = wfm_vals
    ...
    event[["id","image","qtot","qoth","cog","moi","ti","wf_times","qst","qst_all","map","primary","prim_daughter","primary_child_energy","primary_child_pdg","logan_veto","hese_old","hese","weight"]]=(id[0],im,qtot,qoth,[cogPos.x,cogPos.y,cogPos.z],moi,ti,wf2,st_info,st_info_all,emaps,primary[0],prim_daughter[0],energies,pdgs,veto[0],hese[0],hese[1],weight[0])
#    print("AAAA", event['qtot'],event['qoth'],event['cog'],event['moi'],"aa",event["ti"],"bb",event["wf_times"],event['map'],event['hese'],event['logan_veto'],event['hese_old'])
    data.append(event)
